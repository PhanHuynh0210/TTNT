...
static bool gsheetHeaderDone = false;
...
void TaskGGsheet() {
    ...
    if (ready && !gsheetHeaderDone) {
        ...
        FirebaseJson resp;
        bool headerOK = GSheet.values.append(&resp, spreadsheetId, "Sheet1!A:C", &headerRow);
        if (headerOK) {
            Serial.println("Đã ghi tên cột.");
            gsheetHeaderDone = true;
        } else {
            Serial.println("Không ghi được tên cột:");
            Serial.println(GSheet.errorReason());
            // sẽ thử lại ở vòng sau khi token sẵn sàng
            return;
        }
    }
    ...
    if (millis() - lastTime > timerDelay) {
        ...
        bool success = GSheet.values.append(&response, spreadsheetId, "Sheet1!A:C", &valueRange);
        if (success) {
